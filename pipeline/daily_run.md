...
- **Scan for corporate actions** (split/bonus guard): the OHLC store is RAW/unadjusted, so a split halves the close overnight and poisons MA/RSI/ATR/backtest on that series. `--update` now auto-runs `scripts/detect_corporate_actions.py` on the updated symbols and writes candidates to `data/corporate_actions.json`. **If a NEW unconfirmed candidate is flagged for a holding/watchlist name, verify it against PSX filings before trusting that name's technicals this run**, set `confirmed:true` in the file once checked, and note it (this is the same mechanism that caught the UBL Rs10→Rs5 and MTL 2:1 splits behind R-EXP-002). Detection only — never auto-adjust the bars.
