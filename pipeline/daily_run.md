...
- Load `data/portfolio.json`, `data/watchlist.json`, `rules/rules_library.md`, and `data/rolling_stats.json`, and have `pipeline/data_collection.md` ready (Step 1 uses it).
  ...
  - **Ensure the OHLC candle store exists.** `data/ohlc/{SYMBOL}.json` (16 files, ~12 months of real OHLC) powers MA20/50/200, RSI, ATR, and candlestick reads in Step 4. If missing/short — e.g. a fresh clone — backfill first: `.venv\Scripts\python.exe scripts/collect_ohlc.py --backfill --months 12`. Step 1's `--update` keeps it current thereafter.
...
- **Update the OHLC candle store** so technicals run on real history: `.venv\Scripts\python.exe scripts/collect_ohlc.py --update`. This appends today's true OHLC bar (open/high/low/close/volume) for all 16 symbols to `data/ohlc/{SYMBOL}.json`. It is **idempotent** — re-running the same day adds 0 rows. Source = `POST dps.psx.com.pk/historical` (plain `requests`, no browser); the script drops weekend/non-trading rows automatically. (Store + script documented in memory `project_ohlc_store.md`.) If `--update` fails, note it and fall back to the close-only `/timeseries/eod` feed for MAs this run — do not block the pipeline.
