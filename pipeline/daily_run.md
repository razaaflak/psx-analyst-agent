...
- **Scrape today's official EOD via Playwright MCP** from `dps.psx.com.pk`:
  ...
  - Company announcements (results / material info / board meetings). dps is the ONLY live-disclosure source.
- **Build the announcements list with the script — NEVER hand-assemble the URLs** (this is how filings got missed: a hand-built snapshot dropped the PDF link). On the announcements page run the fixed `browser_evaluate` snippet in `scripts/parse_announcements.py`'s docstring (it captures EVERY filing's link — `a[href*="download"]` AND `data-images` — so both `/download/document/<id>.pdf` and `/download/attachment/<id>.pdf` and image-only `.gif` filings are caught). Save the raw rows to a temp json, then:
  `.venv\Scripts\python.exe scripts/parse_announcements.py --raw <rows.json> --date YYYY-MM-DD --merge data/market_data/YYYY-MM-DD.json` — it writes absolute-URL, universe-flagged `announcements[]` into the snapshot and **exits 2 if any filing has no fetchable URL** (block, don't skip). See skill `psx-filings-diligence`.
...
- Write the raw snapshot to `data/market_data/YYYY-MM-DD.json` (+ close screenshot). **Note:** `announcements` should be a **list of `{symbol, title, doc_id, pdf_url}` rows** so the filing processor can fetch each PDF; on a no-filing day a descriptive object is tolerated (the engine handles both). State the data rung + a confidence note. If every automated path fails, skip the run — never fabricate.
- **Process filings into the diligence ledger:** `.venv\Scripts\python.exe scripts/process_filings.py --date YYYY-MM-DD` — downloads each new filing PDF, extracts text, auto-classifies materiality (keyword rules), and persists it in `data/diligence_ledger.json`. **STALE is now ledger-driven, not "filed-today":** a symbol stays STALE while it has an UNRESOLVED filing (MATERIAL or UNKNOWN and not yet reviewed) — this PERSISTS across runs until accounted for (the proper JDWS protection; the old per-run flag self-cleared the next day even if unread). Non-material reaffirmations (rating maintained, routine board notices, AGM minutes, fund distributions, corporate briefings) auto-clear to OK. **For any filing that is MATERIAL or UNKNOWN, open the PDF (render to image + read if image-only / no text layer)** to confirm materiality, then either correct the ledger entry's `materiality` or, once the content is accounted for, clear it: `scripts/process_filings.py --review SYM --notes "..."`. Image-only PDFs are flagged `needs OCR / visual review` and stay STALE until read. `--list-stale` shows the current unresolved set.
...
- **Run the rule scorecard:** `python scripts/universe_signals.py` (no network — reads stored OHLC + fundamentals). For every KSE-100 symbol it emits BUY/HOLD/SELL + screen-grade conviction (1–3) + support/resistance + the `diligence` flag (STALE = an unresolved filing in the Step-1 diligence ledger; persists until reviewed), showing per-factor math. Appends one row/symbol to `data/universe_signals.csv`; sets each row's `grade_on_date`.
